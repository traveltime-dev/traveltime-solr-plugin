...
docker exec -u 0 -d $IMAGE_NAME python3 ./mock-json-server.py 81
...
docker exec $IMAGE_NAME curl -s -o /dev/null -X POST -H 'Content-type:application/json' -d '{"add-queryparser": {"name": "timefilter", "class": "com.traveltime.plugin.solr.TimeFilterQParserPlugin", "api_uri": "http://127.0.0.1:81/v4/", "app_id": "id", "api_key": "key", "prefix": "timefilter_"}}' http://localhost:8983/solr/london/config
docker exec $IMAGE_NAME curl -s -o /dev/null -X POST -H 'Content-type:application/json' -d '{"add-queryparser": {"name": "timefilter_e", "class": "com.traveltime.plugin.solr.TimeFilterQParserPlugin", "api_uri": "http://127.0.0.1:81/v4/", "app_id": "id", "api_key": "key", "cache": "timefilter_exact", "prefix": "timefilter_"}}' http://localhost:8983/solr/london/config
docker exec $IMAGE_NAME curl -s -o /dev/null -X POST -H 'Content-type:application/json' -d '{"add-queryparser": {"name": "timefilter_f", "class": "com.traveltime.plugin.solr.TimeFilterQParserPlugin", "api_uri": "http://127.0.0.1:81/v4/", "app_id": "id", "api_key": "key", "cache": "timefilter_fuzzy", "prefix": "timefilter_"}}' http://localhost:8983/solr/london/config
...
docker exec $IMAGE_NAME solr post -c london part0.json
...
docker exec $IMAGE_NAME \
  curl -s --fail $DATA_ARGS --data-urlencode fq="{!traveltime weight=1}" --data-urlencode "fl=id,score" $URL \
...
docker exec $IMAGE_NAME \
  curl -s --fail $DATA_ARGS --data-urlencode fq="{!traveltime_f weight=1}" --data-urlencode "fl=time:traveltime_f(),id,score" $URL \
  | jq '.response.docs[0].id' | xargs test "n3079325660" ==
...
docker exec $IMAGE_NAME \
  curl -s --fail $DATA_ARGS --data-urlencode fq="{!traveltime_e weight=1}" --data-urlencode "fl=time:traveltime_e(),id,score" $URL \
  | jq '.response.docs[0].id' | xargs test "n3079325660" ==

docker exec $IMAGE_NAME \
  curl -s --fail $DATA_ARGS --data-urlencode fq="{!traveltime_e weight=1}" --data-urlencode "traveltime_distances=true" --data-urlencode "fl=time:traveltime_e(),dist:distance_e(),id,score" $URL \
...
docker exec $IMAGE_NAME \
  curl $TF_DEPARTURE_ARGS --data-urlencode fq="{!timefilter weight=1}" --data-urlencode "fl=id" $URL \
