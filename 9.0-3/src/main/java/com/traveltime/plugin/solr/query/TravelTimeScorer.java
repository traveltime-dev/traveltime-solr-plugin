...
class TravelTimeScorer extends Scorer {
  ...
  private float score(int travelTime) {
    ...
    // >= 0.5 versus 0.0.
    return travelTime == -1.0
        ? 0.0f
        : (float) (boost * (limitAsDouble / (limitAsDouble + travelTime)));
  }

  /**
   * Inverting the score computation is very hard due to all potential rounding errors, so we binary
   * search the maximum travel time. The limit is set to 1 second.
   */
    ...
    PointValues.IntersectVisitor visitor =
        new PointValues.IntersectVisitor() {
          ...
          public PointValues.Relation compare(
              byte[] rangeMinPackedValue, byte[] rangeMaxPackedValue) {
          ...
          private int getTravelTimeForCoordinate(byte[] packedValue) {
            ...
            int rangeMaxLongitudeBits =
                NumericUtils.sortableBytesToInt(packedValue, LatLonPoint.BYTES);
            ...
            return travelTimes == null
                ? -1
                : travelTimes.get(new Coordinates(rangeMaxLat, rangeMaxLon));
          }
        };
