...
class TravelTimeScorer extends Scorer {
   ...
   private float score(int travelTime) {
      ...
      // >= 0.5 versus 0.0.
      return travelTime == -1.0 ? 0.0f : (float) (queryWeight * (limitAsDouble / (limitAsDouble + travelTime)));
   }

   @Override
   public float score() {
      long encodedDocumentCoordinate = docValues.get(docID());
      ...
      return docValues.get(docID()) > 0 ? score(travelTime) : 0;
   }
